...
let meta = {
  ...
  Date: '30/08/2011',
  'Avogadro number': 6.02e23,
  'Radius of the molecule m': 1.85e-8,
...
describe('test parseHeader', () => {
  it('should return an object', () => {
    expect(parseHeader(getMatrixFromXLSX('excel.xls')).meta).toContainEqual(
      meta,
    );
  });
  ...
  it('should return an array with a label and a unit', () => {
    expect(labelUnit('Temperature (m/s)')).toContain(['Temperature ', 'm/s']);
    expect(labelUnit('Temperature m/s')).toContain(['Temperature ', 'm/s']);
    expect(labelUnit('Temperature (Rad) m/s')).toContain([
      'Temperature ',
      'Rad m/s',
    ]);
  });
